...
function App() {
  ...
  const handleVolume = (val) => {
    gainNode.gain.setTargetAtTime(val, context.currentTime, 0.01);
    assignContext();
  };

  const handleBass = (val) => {
    bassEQ.gain.setTargetAtTime(val, context.currentTime, 0.01);
    assignContext();
  };

  const handleMid = (val) => {
    midEQ.gain.setTargetAtTime(val, context.currentTime, 0.01);
    assignContext();
  };

  const handleTreble = (val) => {
    trebleEQ.gain.setTargetAtTime(val, context.currentTime, 0.01);
    assignContext();
  };
  ...
  const overDriveClick = () => {
    ...
    const overDriveOn = !overDriveEl.current.checked;
  ...
  const assignContext = async (c, val) => {
        ...
        <div className="amp--controls">
          ...
          <div className="amp--controls--wrap checkbox--wrap">
            ...
            <input
              ...
              type="checkbox"
              readOnly
              checked={overDrive === false ? "" : "checked"}
            />
